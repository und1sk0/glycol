...
class AircraftType:
    ...
    def to_dict(self) -> Dict:
        ...
        return {"make": self.make, "model": self.model, "notes": self.notes}
...
class TypeGroupsDatabase:
    """Manages the aircraft type groups and glossary database."""

    def __init__(self, db_path: Optional[Path] = None, data_dir: Optional[Path] = None):
        """
        Initialize the type groups database.
    ...
    def save(self):
        ...
        try:
            data = {
                "groups": self.groups,
                "glossary": {
                    code: aircraft.to_dict() for code, aircraft in self.glossary.items()
                },
            }
    ...
    def create_group(
        self, group_name: str, aircraft_codes: Optional[List[str]] = None
    ) -> bool:
        """
        Create a new group.
    ...
    def add_to_group(self, group_name: str, aircraft_code: str) -> bool:
        ...
        if aircraft_code in self.groups[group_name]:
            logger.warning(
                f"Aircraft code {aircraft_code} already in group {group_name}"
            )
            return False
    ...
    def search_types(self, query: str) -> List[AircraftType]:
        ...
        for aircraft in self.glossary.values():
            if (
                query in aircraft.code.lower()
                or query in aircraft.make.lower()
                or query in aircraft.model.lower()
                or query in aircraft.notes.lower()
            ):
